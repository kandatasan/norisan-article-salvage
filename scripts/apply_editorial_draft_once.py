...
USER_AGENT = "tsurikue-editorial-draft-once/1.0"
...
def validate_media(cfg, authorization):
    ...
    if int(cfg["featured_media"]) not in expected: raise RuntimeError("featured_media must be one of expected_media")
...
def validate_target(row,cfg,full):
    ...
    if cfg["editorial_marker"] in current:
        if current.strip()==full.strip() and html.unescape(raw_field(row,"title"))==cfg["title"] and row.get("featured_media")==cfg["featured_media"]: return "ALREADY_UP_TO_DATE"
...
def apply(config_path: Path):
    ...
    if action=="UPDATE":
        payload={"title":cfg["title"],"slug":cfg["slug"],"content":full,"status":"draft","featured_media":int(cfg["featured_media"])}
        response=post_json(f"{SITE_URL}/wp-json/wp/v2/posts/{cfg['post_id']}",auth,payload)
        if response.get("id")!=cfg["post_id"] or response.get("slug")!=cfg["slug"] or response.get("status")!="draft" or response.get("featured_media")!=cfg["featured_media"]: raise RuntimeError("update response validation failed")
    ...
    if after.get("status")!="draft" or after.get("slug")!=cfg["slug"] or after.get("featured_media")!=cfg["featured_media"]: raise RuntimeError("post-update state mismatch")
    ...
    report={"action":action,"post_id":cfg["post_id"],"slug":cfg["slug"],"status":"draft","title":cfg["title"],"featured_media":cfg["featured_media"],"confirmed_media_checked":checked,"public_before":before_counts,"public_after":after_counts,"content_sha256":hashlib.sha256(raw_field(after,"content").encode()).hexdigest(),"wordpress_write_count":1 if action=="UPDATE" else 0,"publish_count":0,"media_upload_count":0}
    out=Path("reports")/f"{cfg['slug']}-draft-update"; out.mkdir(parents=True,exist_ok=True)
    (out/"result.json").write_text(json.dumps(report,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    lines=[f"# {cfg['slug']} draft editorial update","",f"- action: **{action}**",f"- post_id: **{cfg['post_id']}**",f"- status: **draft**",f"- title: {cfg['title']}",f"- featured_media: **{cfg['featured_media']}**",f"- confirmed_media_checked: **{checked}**",f"- public_before: **{before_counts['published_total']}**",f"- public_after: **{after_counts['published_total']}**",f"- content_sha256: `{report['content_sha256']}`"]
