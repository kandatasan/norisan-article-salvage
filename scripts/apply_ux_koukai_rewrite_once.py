...
def replace_optional_once(content: str, old: str, new: str) -> str:
    count = content.count(old)
    if count > 1:
        raise RuntimeError(f"ambiguous text occurrence for {old!r}: {count}")
    return content.replace(old, new, 1) if count == 1 else content
...
def desired_content(current: str) -> str:
    ...
    # Optional tone updates. Missing wording does not block the guarded structural rewrite.
    body = replace_optional_once(
        body,
    ...
    body = replace_optional_once(body, "ここは無理にかばえません。", "ここは、かばえません🤣")
    body = replace_optional_once(body, "タイミングが悪すぎる。", "いや、タイミング悪すぎるって🤣")
