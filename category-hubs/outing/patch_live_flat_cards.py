...
H={'Authorization':'Basic '+token,'Accept':'application/json','Content-Type':'application/json; charset=utf-8','User-Agent':'tsurikue-outing-flat-card-fix/1.0'}
...
content=raw(page)
if page.get('status')!='draft' or MARKER not in content:
    raise SystemExit('REFUSE_WRONG_PAGE_STATE')
original=content

# Remove the nested copy Groups completely. Their constrained/flow layout is what can
# collapse the copy column on desktop. Paragraphs/headings remain native editable blocks.
def flatten(cls,s):
    pat=re.compile(
        rf'<!-- wp:group \{{"className":"{re.escape(cls)}","layout":\{{"type":"(?:constrained|default)"\}}\}} -->\s*'
        rf'<div class="wp-block-group {re.escape(cls)}">(.*?)</div>\s*<!-- /wp:group -->',re.S)
    return pat.subn(lambda m:m.group(1),s)

content,n1=flatten('tq-out-trip-copy',content)
content,n2=flatten('tq-out-route-copy',content)
if n1!=4 or n2!=3:
    raise SystemExit(f'UNEXPECTED_FLATTEN_COUNTS trip={n1} route={n2}')

for cls in ('tq-out-trip-card','tq-out-route-list','tq-out-route-card'):
    content=re.sub(
        rf'(<!-- wp:group \{{"className":"{re.escape(cls)}","layout":\{{"type":")(?:constrained|default)("\}}\}} -->)',
        r'\1default\2',content)

# Remove an earlier copy-column patch if it exists, then append a decisive flat-layout patch.
content=re.sub(r'/\* TQ OUTING FLAT CARD FIX v1 \*/.*?/\* END TQ OUTING FLAT CARD FIX v1 \*/','',content,flags=re.S)
css=r'''
/* TQ OUTING FLAT CARD FIX v1 */
@media(min-width:861px){
  .tq-out .tq-out-trip-card{
    display:grid!important;grid-template-columns:90px minmax(0,1fr)!important;
    grid-template-rows:auto auto auto!important;column-gap:20px!important;row-gap:7px!important;
  ...
  .tq-out .tq-out-route-card{
    display:grid!important;grid-template-columns:155px minmax(0,1fr) 28px!important;
    grid-template-rows:auto auto!important;column-gap:22px!important;row-gap:5px!important;align-items:center!important
  }
...
res=req(f'/pages/{PAGE_ID}','POST',{'content':content})
check=req(f'/pages/{PAGE_ID}?context=edit&_fields=id,status,content')
c=raw(check)
checks={
 'draft':check.get('status')=='draft',
 'marker':MARKER in c,
 'fix':FIX in c,
 'trip_copy_removed':'tq-out-trip-copy' not in c,
 'route_copy_removed':'tq-out-route-copy' not in c,
 'trip_cards':c.count('class="wp-block-group tq-out-trip-card"')==4,
 'route_cards':c.count('class="wp-block-group tq-out-route-card"')==3,
 'temp_nav_absent':'tq-global-site-nav-ref:v1' not in c,
}
