...
SITE='https://tsurikue.com'; POST_ID=3570; UA='tsurikue-verify-emotional-explorer-20260907/1.0'
GULLIVER='[blog_parts id="2843"]'; CTN_BANNER='[blog_parts id="2846"]'; CTN_BUTTON='[blog_parts id="2184"]'; MARKER='<!-- tsurikue-ctn-emotional-explorer-funnel:20260907 -->'
...
def main():
    ...
    assert row['id']==3570 and row['slug']=='lexus-ux-emotional-explorer' and row['status']=='publish' and row['featured_media']==2223
    assert raw(row,'title')=='レクサスUXの特別仕様車エモーショナルエクスプローラーはお得？実際に選んだ理由'
    assert MARKER in c and 'カーセブンとネクステージの2社' in c and '電話が少なくて快適でした' in c
    assert '欲しい1台が見つかったときに動けるよう、今の車の値段も見ておく。' in c
    assert c.count(GULLIVER)==1 and c.count(CTN_BANNER)==1 and c.count(CTN_BUTTON)==1 and problems(c)==0
    assert c.index(MARKER) < c.index(CTN_BANNER) < c.index(CTN_BUTTON)
    print('# lexus-ux-emotional-explorer post-apply verification')
    print('- result: **SUCCESS**')
    print('- wordpress_write_count: **0**')
    print('- status: **publish** / featured_media: **2223**')
    print('- Gulliver **1** / CTN banner **1** / CTN button **1**')
    print('- firsthand phone note + purchase microcopy: **present** / Gutenberg problems: **0**')
