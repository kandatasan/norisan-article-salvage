...
def replace_heading_text(s: str, old: str, new: str) -> str:
    pos = heading_start(s, old)
    end = s.find('<!-- /wp:heading -->', pos)
    if end < 0:
        # Fallback to the end of the actual heading tag.
        tag = re.search(r'<h([1-6])[^>]*>.*?</h\1>', s[pos:], re.S)
        if not tag:
            raise RuntimeError('heading end missing: ' + old)
        end = pos + tag.end()
        block_end = end
    else:
        block_end = end + len('<!-- /wp:heading -->')
    block = s[pos:block_end]
    if old not in block:
        raise RuntimeError('heading text not in selected block: ' + old)
    return s[:pos] + block.replace(old, new, 1) + s[block_end:]
...
def final_build(source: str) -> str:
    ...
    # Preserve every existing article image before touching copy.
    source_images = set(re.findall(r'<img[^>]+src=["\']([^"\']+)["\']', source, re.I))

    out = source

    # 1) Cut the long SEO-style opening, but keep the existing vehicle profile box and photos.
    out = replace_range(
        out,
    ...
    # 2) Sharpen the conclusion and remove the redundant second introduction before defect #1.
...
'''
    ...
    # 3) Keep the detailed firsthand sections, remove only defensive/generic padding.
...
<!-- /wp:paragraph -->''',
    ...
    # 4) Replace the early generic sell-car bridge with CTN's actual advantage.
    out = replace_range(
        out,
    ...
    # 5) Compress repeated "I don't regret it" explanations while preserving trips/photos/ride impressions.
    out = replace_range(
        out,
    ...
    # 6) Sharpen the buyer/affiliate section without changing the FAQ that follows it.
    out = replace_range(
        out,
        heading_start(out, CURRENT_USED_H2),
    ...
    # 7) Sharpen the final landing. FAQ immediately before this remains intact.
    summary_at = heading_start(out, CURRENT_SUMMARY_H2)
    out = out[:summary_at] + m.SUMMARY.strip() + '\n'

    # Structural and preservation guards before any WordPress write.
    expected_counts = {
    ...
    output_images = set(re.findall(r'<img[^>]+src=["\']([^"\']+)["\']', out, re.I))
