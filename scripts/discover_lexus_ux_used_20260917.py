...
SITE='https://tsurikue.com'; POST_ID=2948
raw=f"{os.environ['TSURIKUE_WP_USER']}:{os.environ['TSURIKUE_WP_APP_PASSWORD']}".encode()
auth='Basic '+base64.b64encode(raw).decode()
req=urllib.request.Request(f"{SITE}/wp-json/wp/v2/posts/{POST_ID}?context=edit",headers={'Authorization':auth,'Accept':'application/json','User-Agent':'tsurikue-ux-used-discovery-20260917/1.0'})
