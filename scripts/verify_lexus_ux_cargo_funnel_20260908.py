...
SITE='https://tsurikue.com'; POST_ID=2907; UA='tsurikue-verify-cargo-20260908/1.0'
GULLIVER='[blog_parts id="2843"]'; CTN_BANNER='[blog_parts id="2846"]'; CTN_BUTTON='[blog_parts id="2184"]'; MARKER='<!-- tsurikue-ctn-cargo-funnel:20260908 -->'
...
def main():
    ...
    assert row['id']==2907 and row['slug']=='lexus-ux-cargo' and row['status']=='publish' and row['featured_media']==2209
    assert raw(row,'title')=='レクサスUXの荷室は狭い？ゴルフバッグ・買い物で使えるかをチェック'
    assert MARKER in c and 'カーセブンとネクステージの2社' in c and '電話が少なくて快適でした' in c
    assert '荷室まで納得できたら、次は「今の車、いくらになる？」を見ておく。' in c
    ...
    print('# lexus-ux-cargo post-apply verification')
    print('- result: **SUCCESS**')
    print('- wordpress_write_count: **0**')
    print('- status: **publish** / featured_media: **2209**')
    print('- Gulliver **1** / CTN banner **0** / CTN button **1**')
    print('- firsthand phone note + cargo microcopy: **present before CTN button** / Gutenberg problems: **0**')
