...
SITE='https://tsurikue.com'; POST_ID=2902; UA='tsurikue-verify-rear-seat-20260907/1.0'
GULLIVER='[blog_parts id="2843"]'; CTN_BANNER='[blog_parts id="2846"]'; CTN_BUTTON='[blog_parts id="2184"]'; MARKER='<!-- tsurikue-ctn-rear-seat-funnel:20260907 -->'
...
def main():
    ...
    assert row['id']==2902 and row['slug']=='lexus-ux-rear-seat' and row['status']=='publish' and row['featured_media']==1403
    assert raw(row,'title')=='レクサスUXの後部座席は狭い？大人4人で乗った感想と使い勝手を確認'
    assert MARKER in c and 'カーセブンとネクステージの2社' in c and '電話が少なくて快適でした' in c
    assert '後席まで納得できたら、次は「今の車、いくらになる？」を見ておく。' in c
    ...
    print('# lexus-ux-rear-seat post-apply verification')
    print('- result: **SUCCESS**')
    print('- wordpress_write_count: **0**')
    print('- status: **publish** / featured_media: **1403**')
    print('- Gulliver **1** / CTN banner **0** / CTN button **1**')
    print('- firsthand phone note + rear-seat microcopy: **present before CTN button** / Gutenberg problems: **0**')
