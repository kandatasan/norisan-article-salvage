...
UA = "tsurikue-fj-title-salvage-drafts-20260924/1.0"
...
DRAFTS = [
    {
        ...
        "featured_media": 0,
        "content_sha256_stripped": "aab8a207df8bfbda31b3ce2aafda46866f7603b471726e4210fc40fb11585b2e",
    ...
    },
    {
        ...
        "featured_media": 0,
        "content_sha256_stripped": "c895bdd5f508f56af93842da2160def2ae57a3545b7b4b44e17f04b7c8d647f4",
...
def get_post(pid: int):
    q = urllib.parse.urlencode({
        "context": "edit",
        "_fields": "id,slug,status,title,content,featured_media,date_gmt,modified_gmt",
    })
...
def validate_media(media: dict[int, str]):
    for media_id, expected_path in media.items():
        ...
        if actual_path != expected_path.casefold():
            raise RuntimeError(
                f"media mismatch id={media_id}: {actual_path} != {expected_path}"
            )

def validate_identity(row, target):
    ...
    if row.get("status") != target["status"]:
        raise RuntimeError(f"status mismatch: {target['slug']}")
    if html.unescape(raw(row, "title")) != target["old_title"]:
...
def main():
    ...
    # Preflight FJ title-only update.
    fj_row = get_post(FJ["id"])
    validate_identity(fj_row, FJ)
    fj_content = raw(fj_row, "content")
    if sha_stripped(fj_content) != FJ["content_sha256_stripped"]:
        raise RuntimeError("FJ content changed since inspected snapshot; refusing title update")
    if block_problems(fj_content):
        raise RuntimeError("FJ Gutenberg structure invalid before update")
    prepared.append({
        "target": FJ,
    ...
    # Preflight ALL draft salavages before any WordPress write.
    for target in DRAFTS:
        row = get_post(target["id"])
        validate_identity(row, target)
        current = raw(row, "content")
        if sha_stripped(current) != target["content_sha256_stripped"]:
            raise RuntimeError(
                f"{target['slug']} draft changed since inspected snapshot; refusing overwrite"
            )
        if target["salvage_marker"] not in current:
            raise RuntimeError(f"salvage marker missing: {target['slug']}")
        if target["editorial_marker"] in current:
            raise RuntimeError(f"new editorial marker already present: {target['slug']}")
        if block_problems(current):
            raise RuntimeError(f"Gutenberg structure invalid before: {target['slug']}")
        ...
        if block_problems(new_content):
            raise RuntimeError(f"Gutenberg structure invalid after local build: {target['slug']}")
        if count_images(current) != target["expected_images"]:
            raise RuntimeError(
                f"unexpected current image count {target['slug']}: "
                f"{count_images(current)} != {target['expected_images']}"
            )
        if count_images(new_content) != target["expected_images"]:
            raise RuntimeError(
                f"new image count mismatch {target['slug']}: "
                f"{count_images(new_content)} != {target['expected_images']}"
            )
    ...
    try:
        for item in prepared:
            ...
            if after.get("id") != target["id"] or after.get("slug") != target["slug"]:
                raise RuntimeError(f"post-update identity mismatch: {target['slug']}")
            if after.get("status") != target["status"]:
                raise RuntimeError(f"post-update status mismatch: {target['slug']}")
            if int(after.get("featured_media") or 0) != target["featured_media"]:
                raise RuntimeError(f"post-update featured_media mismatch: {target['slug']}")
            if html.unescape(raw(after, "title")) != target["new_title"]:
                raise RuntimeError(f"post-update title mismatch: {target['slug']}")
            if after.get("date_gmt") != item["before_row"].get("date_gmt"):
                raise RuntimeError(f"original publication date changed: {target['slug']}")
            ...
            if item["kind"] == "published-title-only":
                if sha_stripped(actual) != target["content_sha256_stripped"]:
                    raise RuntimeError("FJ content changed during title-only update")
            else:
                if actual.strip() != item["new_content"].strip():
                    raise RuntimeError(f"draft content mismatch after write: {target['slug']}")
                if count_images(actual) != target["expected_images"]:
                    raise RuntimeError(f"image count changed after write: {target['slug']}")
                if count_blog_parts(actual) != count_blog_parts(item["before_content"]):
                    raise RuntimeError(f"blog_parts changed after write: {target['slug']}")
                if has_h1(actual):
        ...
        # Best-effort rollback to the exact preflight state.
    ...
    if before_public != after_public:
        raise RuntimeError(
            f"published counts changed: {before_public} -> {after_public}"
        )

    print("# FJ title micro-rewrite + two draft salvages 2026-09-24")
    print("- result: **SUCCESS**")
    print(
        f"- public posts: **{before_public['posts']} → {after_public['posts']}**"
    )
    print(
        f"- public pages: **{before_public['pages']} → {after_public['pages']}**"
    )
    print("- FJ cheap: **publish → publish / title only / content unchanged**")
    print("- karaagekariju: **draft → draft / full salvage applied**")
    print("- yakitori-riku: **draft → draft / full salvage applied**")
    ...
    print("- modified date: **updated on all 3**")
