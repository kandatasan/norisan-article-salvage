...
REPORT_DIR = Path("reports/ux-koukai-rewrite-audit-retry")
USER_AGENT = "tsurikue-ux-koukai-rewrite-audit-retry/1.0"
...
def get_json(url: str, auth: str):
    ...
    raise RuntimeError(f"GET failed after 3 attempts: {last}")
...
def main() -> int:
    ...
    try:
        ...
        new_presence = {marker: marker in content for marker in NEW_MARKERS}
        old_presence = {marker: marker in content for marker in OLD_MARKERS}
        counts = {
        ...
        applied = all(new_presence.values()) and not any(old_presence.values()) and counts == EXPECTED_COUNTS
        report.update({
