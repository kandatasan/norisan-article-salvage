...
SITE='https://tsurikue.com'; POST_ID=2975; UA='tsurikue-verify-lexus-ux-model-change-20260907/1.0'
GULLIVER='[blog_parts id="2843"]'; CTN_BANNER='[blog_parts id="2846"]'; CTN_BUTTON='[blog_parts id="2184"]'; MARKER='<!-- tsurikue-ctn-model-change-funnel:20260907 -->'
...
def main():
    ...
    assert row['id']==2975 and row['slug']=='lexus-ux-model-change' and row['status']=='publish' and row['featured_media']==2244
    assert raw(row,'title')=='レクサスUXのモデルチェンジはいつ？次期型は出る？生産終了の噂も整理'
    assert MARKER in c and '私のときは2社から連絡が来て、電話が少なくて快適でした' in c and '待つか、今買うか。その前に「今の車、いくらになる？」を見ておく。' in c
    ...
    print('# lexus-ux-model-change post-apply verification')
    print('- result: **SUCCESS**')
    print('- wordpress_write_count: **0**')
    print('- status: **publish** / featured_media: **2244**')
    print('- Gulliver **1** / CTN banner **0** / CTN button **1**')
    print('- firsthand phone note + model-change microcopy: **present before CTN button** / Gutenberg problems: **0**')
